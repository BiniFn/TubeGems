...
def fallback_download():
    ...
    filename_ext = 'mp3' if type_ == 'audio' else 'mp4'
    safe_title = sanitize_filename(raw_title).strip()[:120] or 'video'
    filename = f"{safe_title}.{filename_ext}"

    if probe:
        return jsonify({"ok": True, "filename": filename, "url": candidate})
    ...
    return Response(stream_with_context(stream_proxy(candidate)), headers=resp_headers)
...
def stream_proxy(url, headers=None):
    try:
        request_headers = {'User-Agent': 'TubeGems/1.0'}
        if headers:
            request_headers.update(headers)

        with requests.get(url, stream=True, headers=request_headers, timeout=20) as external_req:
